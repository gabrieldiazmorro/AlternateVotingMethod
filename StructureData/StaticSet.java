...
public class StaticSet<E> implements Set<E> {
	...
	public boolean equals(Set<E> S2) {
		...
		return (this.difference(S2).isEmpty() && S2.difference(this).isEmpty());
	}
	...
	public static boolean checkDisjoint (Object[] sets) {
		Set<Integer> iterator = (Set<Integer>) sets[0];
		for (int i = 1; i < sets.length; i++) {
			if (iterator.intersection((Set<Integer>)sets[i]).isEmpty()) {
				return true;
			}
			iterator =(Set<Integer>) iterator.intersection((Set<Integer>)sets[i]);
		}
		return false;
	}
